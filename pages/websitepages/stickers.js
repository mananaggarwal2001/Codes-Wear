                ...
                <div className="container px-5 py-24 ">
                    <div className="flex flex-wrap items-center">
                        {Object.keys(products).length === 0 && <div className='mx-auto'>
                            <div className='font-semibold'>Sorry all the stickers are out of Stock. New Stock comming Soon.Stay Tuned!!!!!!!
                            </div>
                        </div>
                        }
                        {
...
export async function getServerSideProps(context) {
    let finalproducts = await Product.find({ category: 'stickers' }) // for finding all the products from the database.
    let sticks = {};
    for (let item of finalproducts) {
        if (item.title in sticks) {
            if (!sticks[item.title].color.includes(item.color) && item.avaiableQty > 0) {
                sticks[item.title].color.push(item.color)
            }

            if (!sticks[item.title].size.includes(item.size) && item.avaiableQty > 0) {
